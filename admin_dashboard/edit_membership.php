...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE memberships SET name = ?, description = ?, price = ?, duration_months = ?, features = ?, status = ? WHERE membership_id = ?");
        $stmt->bind_param("ssdissi", $name, $description, $price, $duration_months, $features, $status, $membership_id);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Error updating membership plan: " . $conn->error;
        }
    }
}
...
if ($result->num_rows === 0) {
    $_SESSION['error_message'] = "Membership plan not found.";
    header("Location: manage_membership.php");
    exit();
}
...
<head>
    ...
    <title>Edit Membership Plan - FitZone Admin</title>
...
<body>
    <div class="dashboard">
        ...
        <div class="main-content">
            ...
            $subscriber_count = $subscriber_result->fetch_assoc()['count'];
            ?>

            <div class="subscriber-count">
                <i class="fas fa-users"></i>
                <span>This plan has <?php echo $subscriber_count; ?> active subscriber<?php echo $subscriber_count !== 1 ? 's' : ''; ?>.</span>
            </div>
            ...
            <div class="form-container">
                <form method="POST" action="" id="membershipForm">
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="<?php echo htmlspecialchars($membership['name']); ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="description">Description</label>
                        <textarea id="description" name="description" class="form-control" required><?php echo htmlspecialchars($membership['description']); ?></textarea>
                    </div>

                    <div class="form-group">
                        <label for="price">Price (Rs.)</label>
                        <input type="number" id="price" name="price" class="form-control" step="0.01" min="0" value="<?php echo htmlspecialchars($membership['price']); ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="duration_months">Duration (months)</label>
                        <input type="number" id="duration_months" name="duration_months" class="form-control" min="1" value="<?php echo htmlspecialchars($membership['duration_months']); ?>" required>
                    </div>
                        ...
                        <div class="features-container" id="featuresContainer">
                            <?php foreach ($features as $feature): ?>
                                <div class="feature-input">
                                    <input type="text" name="features[]" class="form-control" value="<?php echo htmlspecialchars(trim($feature)); ?>" required>
            ...
            <div class="preview-card" id="previewCard">
                <div class="preview-header">
                    <h3 id="previewName"><?php echo htmlspecialchars($membership['name']); ?></h3>
                    <div id="previewPrice">$<?php echo number_format($membership['price'], 2); ?></div>
                    <div id="previewDuration"><?php echo $membership['duration_months']; ?> months</div>
                </div>
                <div class="preview-body">
                    <p id="previewDescription"><?php echo htmlspecialchars($membership['description']); ?></p>
